...
public class Main {
    ...
    public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        char[][] map = new char[h][h];
        boolean [][][] vis = new boolean [4][h][h];        
            ...
            for(int j=0;j<h;j++) {                
                if(map[i][j]=='#') {                   
                    for(int d=0;d<4;d++) {
                        ...
                        int nr = dr[d]+i;
                        int nc = dc[d]+j;
                        if(nr<0||nc<0||nr>=h||nc>=h||map[nr][nc]=='*')continue;
                        if(map[nr][nc]=='#') {                            
...
//            System.out.println(x.x+" "+x.y+" "+x.mr);
            ...
            if(nr>=0&&nc>=0&&nr<h&&nc<h&&map[nr][nc]!='*'&&!vis[x.ar][nr][nc]) {                
                pq.add(new point(nr,nc,x.ar,x.mr));                
            }
            if(map[x.x][x.y]=='.')continue;
            if(nr1>=0&&nc1>=0&&nr1<h&&nc1<h&&map[nr1][nc1]!='*'&&!vis[(x.ar+1)%4][nr1][nc1]) {
                pq.add(new point(nr1,nc1,(x.ar+1)%4,x.mr+1));                
            }
            if(nr2>=0&&nc2>=0&&nr2<h&&nc2<h&&map[nr2][nc2]!='*'&&!vis[(x.ar+3)%4][nr2][nc2]) {
